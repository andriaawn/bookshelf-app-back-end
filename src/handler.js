...
const showAllBooksHandler = (request, h) => {
  ...
  if (name !== undefined) {
    const isname = name === 'Dicoding';
    if (isname) {
      return {
        status: 'success',
        data: {
          books: books.filter((book) => book.name.toLowerCase()
            .includes(name.toLowerCase())).map((a) => ({
            id: a.id,
            name: a.name,
  ...
  if (finished !== undefined) {
    ...
    if (isFinished) {
      return {
        status: 'success',
        data: {
          books: books.filter((book) => book.finished === true).map((c) => ({
            id: c.id,
            name: c.name,
    ...
    if (!isFinished) {
      return {
        status: 'success',
        data: {
          books: books.filter((book) => book.finished === false).map((c) => ({
            id: c.id,
            name: c.name,
            publisher: c.publisher,
          })),
  ...
  const showAllBooks = books.map((book) => ({
    id: book.id,
    name: book.name,
